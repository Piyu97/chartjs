    ...
    function data() {
        fetch("https://raw.githubusercontent.com/anadahalli/interview/master/db.json")
            ...
            .then(res => {
                ...
                orderArr.forEach((item,i)=>priceObj[item.date]=(priceObj[item.date]===undefined?0:priceObj[item.date]+Number(item.price)))
    ...
    return (
        <React.Fragment>
            ...
            <select onClick={(e) => setPerPage(e.target.value)}>
                ...
                <option value={prod.length}>All Data</option>
            </select>



             {Object.keys(orderCount).length>=1&&<Line data={orderCount}></Line>}
             {Object.keys(orderCount).length>=1&&<Bar data={priceCount}></Bar>}
